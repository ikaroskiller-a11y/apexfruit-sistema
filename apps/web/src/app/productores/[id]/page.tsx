...
import { eliminarProductor } from "../actions";
...
export default async function ProductorDetallePage({
  ...
  const [productor, usuario] = await Promise.all([
    prisma.productor.findUnique({
      where: { id },
      include: {
        lotes: {
  ...
  const filas = productor.lotes.map((lote) => {
  ...
  return (
    <>
      <TopBar title={productor.nombre} />
      <main className="flex-1 space-y-6 px-4 py-6 md:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link href="/productores" className="text-sm text-brand-700 hover:underline dark:text-brand-500">
            ← Volver a productores
          </Link>
          {puedeEditar ? (
            <div className="flex items-center gap-2">
              <Link
                href={`/productores/${productor.id}/editar`}
              ...
              <BotonEliminar
                ...
                hiddenFields={{ productorId: productor.id }}
                confirmMessage={`¿Eliminar el productor "${productor.nombre}"? Esta acción no se puede deshacer.`}
              />
            </div>
          ) : null}
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-1">
            ...
            <dl className="space-y-2 text-sm">
              <Row label="RUT" value={productor.rut ?? "—"} mono />
              <Row label="Contacto" value={productor.contacto ?? "—"} />
              <Row label="Email" value={productor.email ?? "—"} />
              <Row label="Teléfono" value={productor.telefono ?? "—"} mono />
              <Row label="Dirección" value={productor.direccion ?? "—"} />
            </dl>
          </Card>

          <Card className="lg:col-span-2 !p-0">
            <div className="border-b border-border px-5 py-4">
              <CardTitle>Lotes ({productor.lotes.length})</CardTitle>
            </div>
                  ...
                  {filas.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-4 py-8 text-center text-fg-muted">
                        Este productor todavía no tiene lotes.
                      </td>
                    </tr>
                  ) : null}
                </tbody>
        ...
        {productor.notas ? (
          <Card>
            ...
            <p className="text-sm text-fg">{productor.notas}</p>
          </Card>
        ) : null}
      </main>
    </>
